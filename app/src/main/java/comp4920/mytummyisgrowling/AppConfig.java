package comp4920.mytummyisgrowling;

/**
 * Created by Ken on 10/1/2015.
 */
public class AppConfig {
    // Server user login url
    public static String URL_LOGIN = "http://60.240.39.122:8080/login_api/login.php";

    // Server user register url
    public static String URL_REGISTER = "http://60.240.39.122:8080/login_api/register.php";

    // Server user register url
    public static String URL_EDITPREF = "http://60.240.39.122:8080/login_api/editPrefs.php";

    // Server avatar upload url
    public static String URL_UPLOADAVATAR = "http://60.240.39.122:8080/login_api/avatar.php";

    public static String URL_CREATE_GROUP = "http://60.240.39.122:8080/login_api/groups.php";
}